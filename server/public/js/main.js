...
const h1_private_key = document.querySelector('.header__private-key-text')


next_btn.onclick = function() {
    next_btn.style.left = "200%";
    h1_private_key.style.left = "16%"
    h1_private_key.innerHTML = "Confirm the secret phrase"
    next_btn.style.visibility = "hidden";
    show_private_key_btn.style.visibility = "hidden"
    copy_btn.style.visibility = "hidden";
    send_btn.style.visibility = "visible";
    show_private_key_btn.style.left = "200%";
    copy_btn.style.left = "200%";
    for (let index = 0; index < private_key_el.length; index++) {
        const element = private_key_el[index];
        element.style.visibility = "hidden"
    }
...
send_btn.onclick = function () {
    ...
    var succes = []
    var error = false
    arr = private_key_str.value.split(' ')
    for (let index = 0; index < confirm_private_key.length; index++) {
      const element = confirm_private_key[index].value;
        if(element != arr[index])
        {
            error = true;
            back_btn.style.visibility = "visible"
            send_btn.style.border = "2px solid red"
            back_btn.style.border = "2px solid red"
            back_btn.style.left = "37%"
            errors_h2.style.left = "26%"
            errors_h2.style.visibility = "visible"
            break;
        }
    }
    if(error == false)
    {
        private_key_active.style.left = '200%';
        create_finish_block.style.visibility = "visible"
